...
export default class Universities extends Component {
  ...
  uniDetails = (uni_id) => {
    ...
    axios
      .get(
        "https://virtserver.swaggerhub.com/MuhammetDilmac/Intern/1.0.0/universities/id" +
          uni_id
      )
      .then(Response => {
        const uni_id = Response.data.id;
        const uni_name = Response.data.name;
        const uni_city = Response.data.city;
        const uni_founded_at = Response.data.founded_at;
        const uni_web_page = Response.data.web_page;
        const uni_type = Response.data.type;
        this.setState({ uni_id: uni_id });
        this.setState({ uni_name: uni_name });
        this.setState({ uni_city: uni_city });
